...
def showprofils(sm, figure=None, visu=1, Data=None, bmus=None, scale=None, \
    ...
    if ColorClass is None :
        ...
        ###>>>
        if 0 :
            cmap       = cm.jet; colcellmax=6 #!!!!!!!
            ColorClass = cmap(np.arange(1,256,round(256/colcellmax))); #.astype(int)));
            ColorRef   = ColorClass[Clevel-1]
            print(np.shape(ColorRef))
        if 0 :
            #colcellmax = 7; 
            colcellmax = int(max(Clevel)+1);
            cmap       = cm.jet;
            #col       = cmap(np.arange(1,256,round(256/colcellmax).astype(int)));
        #   ColorClass = cmap(np.arange(1,256,round(256/colcellmax)));
            ColorClass = cmap(np.arange(1,256,round(256/colcellmax)));
        #   ColorClass[:,3] = 0.5
            print(colcellmax); print(ColorClass)
            #ColorRef   = ColorClass[Clevel-1]
        if 0 :
            print("HELLO");
            #Clevel     = np.array([1,2,3,4,5,6])-1
            Clevel     = Clevel-1
            ColorClass = np.array([[0, 0, 102],
                                   [30, 70, 239],
                                   [0, 255, 204],
                                   [204, 255,102],
                                   [255,102,0],
